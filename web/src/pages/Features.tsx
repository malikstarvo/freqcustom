    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold flex items-center gap-2">
          <Sparkles className="text-accent" /> Features
        </h2>
        <div className="flex items-center gap-2">
          <span className="text-xs text-text-secondary">{strategies.length} strategies</span>
          <span className="text-xs text-text-secondary">{freqaimodels.length} models</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card title="Strategy Registry">
          <div className="space-y-2">
              ...
              <button
                ...
                className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors ${
                  selectedStrategy === s
                    ...
                    : "bg-card-bg border border-card-border text-text-primary hover:border-accent/30"
                }`}
              >
                <div className="flex items-center gap-2">
                  <Zap size={14} />
            ...
            {strategies.length === 0 && (
              <p className="text-sm text-text-secondary">No strategies found</p>
            )}
          </div>
        </Card>
        ...
        <Card title="FreqAI Model Registry">
          <div className="space-y-2">
            {freqaimodels.map((m) => (
              <div key={m} className="flex items-center gap-2 px-3 py-2 bg-card-bg rounded-lg border border-card-border text-sm">
                <Brain size={14} className="text-accent" />
                <span className="font-medium">{m}</span>
              </div>
            ))}
            {freqaimodels.length === 0 && (
              <p className="text-sm text-text-secondary">No FreqAI models found</p>
            )}
          </div>
        </Card>
      </div>
        ...
        <Card title={`Strategy Parameters: ${selectedStrategy}`}>
          {loading ? (
            ...
            <div className="space-y-4">
              ...
              <div className="border border-card-border rounded-lg overflow-auto">
                  ...
                  <tbody>
                    {params?.map((p, i) => (
                      <tr key={i} className="border-b border-card-border/30">
                        ...
                        <td className="px-3 py-2 text-text-secondary">{p.param_type}</td>
                        ...
                        <td className="px-3 py-2">
                          <span className={`text-xs px-2 py-0.5 rounded ${p.opt_range ? "bg-accent/10 text-accent" : "bg-gray-800 text-gray-400"}`}>
                            {p.opt_range ? "Yes" : "No"}
                          </span>
                        </td>
                      </tr>
                    ...
                    {(!params || params.length === 0) && (
                      <tr><td colSpan={5} className="px-3 py-4 text-center text-text-secondary">No parameters</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
            ...
            <p className="text-sm text-text-secondary">Failed to load strategy details</p>
          )}
        </Card>
      )}
      ...
      <Card title="Edge Study">
        <div className="space-y-3">
          ...
          <div className="p-3 bg-accent/5 rounded-lg border border-accent/20 text-sm">
            <span className="text-accent font-medium">CLI Command: </span>
            <code className="text-text-secondary">freqtrade edge-study --symbol BTC/USDT --timeframe 1h --output edge_study_results</code>
          </div>
        </div>
      </Card>
    </div>
