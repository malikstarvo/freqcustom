...
export default function Layout() {
  ...
  return (
    <div className="flex h-screen">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed lg:static inset-y-0 left-0 z-50 w-56 bg-[--color-card-bg] border-r border-[--color-card-border] flex flex-col transition-transform duration-200 lg:translate-x-0 ${
      ...
      >
        <div className="p-4 border-b border-[--color-card-border] flex items-center justify-between">
          ...
          <button
            onClick={closeSidebar}
            className="lg:hidden p-1 text-[--color-text-secondary] hover:text-[--color-text-primary]"
          >
            <X size={20} />
          </button>
        </div>
        ...
        <nav className="flex-1 py-4 px-2 space-y-1 overflow-y-auto">
          {navItems.map(({ path, label, icon: Icon }) => (
            <NavLink
              ...
              onClick={closeSidebar}
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-2 rounded-md text-sm transition-colors ${isActive ? "bg-[--color-accent]/10 text-[--color-accent]" : "text-[--color-text-secondary] hover:bg-gray-800 hover:text-white"}`
              }
            >
              <Icon size={18} />
              {label}
        ...
        <div className="p-4 border-t border-[--color-card-border] flex items-center justify-between">
          ...
          <button
            onClick={toggle}
            className="p-1.5 rounded-md text-[--color-text-secondary] hover:bg-[--color-card-border] hover:text-[--color-text-primary] transition-colors"
      ...
      <main className="flex-1 overflow-auto p-4 lg:p-6 min-w-0">
        ...
        <div className="lg:hidden flex items-center gap-3 mb-4">
          <button
            ...
            className="p-2 rounded-md bg-[--color-card-bg] border border-[--color-card-border] text-[--color-text-secondary]"
          >
            <Menu size={20} />
          </button>
